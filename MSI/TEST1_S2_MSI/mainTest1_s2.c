...
#define NOM_DEVELOPPEUR "MATTEO STEFANELLI"
#define SEMESTRE		 2
...
void main()
{
	...
	do
	{
		...
		if (validationSerie == NOT_OK)
		{
			...
			validationSerie = ControleChoixSerie(choixUserSerie);													//(controle choix serie)
		}
		...
		if (validationResistance == NOT_OK)
		{
			...
			validationResistance = ControleValR(choixUserValResistance);										//controle Val R
		}
		...
		if (validadationPuissanceR == NOT_OK)
		{
			...
			//-- appel de fonction => test si valeur résistance unitaire est dans les limites --// 
			validadationPuissanceR = ControlePoidPuissanceR(choixPuissanceR);												//controle poids puissance
		}
	...
	Resistances.resistanceBrute = CalculRUser(choixUserValResistance, choixPuissanceR);											//calcul R user
	...
	CalculValSerie(&Resistances);																							//controle val serie

	//-- calcul de la valeur normalisée --// 
	CalculRNormalisee(&Resistances);																						// calcul normalisee
